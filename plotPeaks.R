...
# Select top N peaks per cell type

# --------------------------------------------------------------------

top_peak_ids <- unique(unlist(lapply(cell_order, function(cl) {
cl_scores <- mat_z[, cl]
top_idx <- order(abs(cl_scores), decreasing = TRUE)[1:min(args$n, length(cl_scores))]
rownames(mat_z)[top_idx]
})))
